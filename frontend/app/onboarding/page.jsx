...
export default function OnboardingPage() {
  ...
  async function handleNext() {
    ...
    if (step === 3) {
      ...
      setSubmitting(true);
      try {
        await ProfileApi.updateMe(user.documentId, { country: form.countryId });
        if (form.profileType === "skilled") {
          await ProfileApi.createSkilled({
            national_id_number: form.nationalId,
            skills_category: form.skillsCategory,
          ...
          await ProfileApi.createProfessional({ user: user.documentId });
        }
        setStep(4);
      } catch (err) {
        setError(err.message);
        ...
        <Stack direction="column" gap={2}>
          ...
          <Stack gap={1} wrap="wrap">
            {skills
              .filter((s) => s.category === form.skillsCategory)
              .map((s) => {
                const active = form.skillIds.includes(s.documentId);
                return (
                  <Chip
                    key={s.documentId}
                    label={s.name}
                    ...
                    onClick={() =>
                      update(
                        "skillIds",
                        active
                          ? form.skillIds.filter((id) => id !== s.documentId)
                          : [...form.skillIds, s.documentId]
                      )
                    }
                  />
                );
