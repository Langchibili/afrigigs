...
export default function PostJobPage() {
  ...
  const selectedCountry = countries.find((c) => c.documentId === form.countryId);
  ...
  async function handleSubmit(e) {
    ...
    try {
      const jobRes = await JobApi.create({
        ...
        target_city: form.targetCity,
        scope: form.scope,
        budget_currency: selectedCountry?.currency?.documentId,
        budget_amount: Number(form.budgetAmount),
        skills_required: form.skillIds,
        job_status: "open",
      });

      const jobId = jobRes.data.documentId;
      ...
      await WalletApi.fund({
        job: jobId,
        amount: Number(form.budgetAmount),
        currency: selectedCountry?.currency?.documentId,
        gateway: form.gateway,
        gateway_ref: `pending_${Date.now()}`,
      });
          ...
          <Stack gap={0.75} wrap="wrap">
            {skills.map((s) => (
              <Chip
                ...
                label={s.name}
                size="small"
                color={form.skillIds.includes(s.documentId) ? "primary" : "default"}
