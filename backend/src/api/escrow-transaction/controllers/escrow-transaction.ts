...
export default factories.createCoreController('api::escrow-transaction.escrow-transaction', ({ strapi }) => ({
    async release(ctx) {
        try {
            ...
            if (transaction.escrow_transaction_status !== 'held' && transaction.escrow_transaction_status !== 'funded') {
                return ctx.badRequest(`Transaction is already ${transaction.escrow_transaction_status}.`);
            }
            ...
            await strapi.db.query('api::escrow-transaction.escrow-transaction').update({
                where: { id },
                data: { escrow_transaction_status: 'released' },
            });
